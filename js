...
const SCHNAPS = [222, 333, 444]; // Schnapszahlen
...
/*** NAME → Basen (aus deiner Liste, kuratiert) ***/
const NAME_BASES = {
  "Adonis*": ["Wine"],
...
/*** Aus NAME_BASES eine Seiten-Mapping-Tabelle bauen ***/
const RECIPE_BASES = {};
for (const {name, page} of RECIPES){
  const bases = NAME_BASES[name] || [];
  RECIPE_BASES[page] = [...bases];
}

/*** Multi-Primary bestimmen: ≥2 Primärspirituosen (oder Spirit + Bubbles/Wine mischt) ***/
const MULTI_PRIMARY = new Set();
for (const {name, page} of RECIPES){
  const bases = NAME_BASES[name] || [];
  const primCount = bases.filter(b => PRIMARY.has(b)).length;
  const hasWineOrBubbles = bases.includes("Wine") || bases.includes("Bubbles");
  if (primCount >= 2) MULTI_PRIMARY.add(page);
  // Optional: wer Spirit + Bubbles/Wine enthält, bleibt erlaubt,
  // solange NUR 1 Primärspirituose drin ist (z. B. French 75: Gin + Bubbles).
}
...
let tempExcludes = new Set(); // nur nächste Ziehung
...
function initOverlay(){
  document.querySelectorAll('#overlay .opt').forEach(opt => {
    const base = opt.dataset.base; const pin = opt.querySelector('.pin');
  ...
  goBtn.addEventListener('click', () => {
    overlay.classList.remove('show');
    nextDrawExcludes = [...tempExcludes]; // nur nächste Ziehung
    tempExcludes.clear(); syncOverlayUI();
    suppressNextClick = false;
  });

  clearBtn.addEventListener('click', () => {
    barExcludes.clear(); tempExcludes.clear(); syncOverlayUI(); saveState();
  });
}
function openOverlay(){ syncOverlayUI(); overlay.classList.add('show'); }
...
function finishDraw(temp = []){
  ...
  if (SCHNAPS.includes(lastRolledNumber)){
    ...
    cocktailEl.textContent = rec ? rec.name : "Schnapszahl";
  ...
  // Erlaubte Basen = alles minus barExcludes minus tempExcludes
  const allowed = new Set(BASES.filter(b => !barExcludes.has(b) && !temp.includes(b)));
  const anyFilterActive = (allowed.size !== BASES.length);

  // Strenger Filter auf Basis der Mapping-Tabelle
  const filtered = pool.filter(p => {
    if (!anyFilterActive) return true; // keine Filter → alles
    const bases = RECIPE_BASES[p] || [];
    if (!bases.length) return false;               // ohne Mapping raus
    if (MULTI_PRIMARY.has(p)) return false;        // ≥2 Primärspirituosen → raus
    return bases.every(b => allowed.has(b));       // alle Basen müssen erlaubt sein
  });
...
function celebrateSchnaps(){
  ...
  megaConfetti(); launchRockets(4);
  setTimeout(()=> numberEl.classList.remove('schnapszahl'), 3000);
}

/*** STATE / RESET ***/
function renderHistory(){ historyEl.textContent = history.length ? ("Letzte: " + history.join(" • ")) : ""; }
function saveState(){
...
function checkDailyReset(){
  ...
  const dayKey = x => { const y=new Date(x); if (y.getHours() < RESET_HOUR) y.setDate(y.getDate()-1); return y.toDateString(); };
  if (dayKey(now) !== dayKey(d)){
    pool = shuffle([...RECIPE_PAGES]);
    history = [];
    if (now.getHours() >= RESET_HOUR) barExcludes.clear();
    saveState();
  }
}
function loadBarExcludes(){
  try { const raw = localStorage.getItem(LS_BAR_FILTER_KEY); if (!raw) return new Set(); const arr = JSON.parse(raw); return new Set(Array.isArray(arr)?arr:[]); }
...
/*** Konfetti & Raketen ***/
const canvas=document.getElementById('confetti'),ctx=canvas.getContext('2d',{alpha:true});
...
function confettiBurst({count=240, spread=Math.PI/2, pow=1, originX=.5, originY=.22} = {}){
  ...
  for(let i=0;i<count;i++){
    const angle=(-Math.PI/2)+(Math.random()-.5)*spread, speed=(6+Math.random()*7)*pow;
    particles.push({x:ox,y:oy,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed-2,g:.22+Math.random()*.18,
      size:2+Math.random()*3.5,color:GOLD,life:maxLife-Math.random()*30,rot:Math.random()*Math.PI,vr:(Math.random()-.5)*.35});
  }
}
...
function launchRockets(n=3){
  ...
  for(let i=0;i<n;i++){
    rockets.push({x:W*(.25+.5*Math.random()),y:H*1.05,vx:(Math.random()-.5)*1.2,vy:-(10+Math.random()*4),tY:H*(.18+.1*Math.random()),
      trail:[],life:220,hue:GOLD});
  }
}
(function render(){
  ...
  for (let i=particles.length-1;i>=0;i--){
    ...
    ctx.save(); ctx.translate(p.x,p.y); ctx.rotate(p.rot); ctx.fillStyle=p.color; ctx.globalAlpha=Math.max(0,p.life/110);
  ...
  for (let i=rockets.length-1;i>=0;i--){
    const r=rockets[i]; if (r.life<=0){ rockets.splice(i,1); continue; }
    r.vy+=.12; r.x+=r.vx; r.y+=r.vy; r.life--; r.trail.push({x:r.x,y:r.y}); if (r.trail.length>24) r.trail.shift();
    ctx.beginPath(); for(let j=0;j<r.trail.length;j++){ const p=r.trail[j]; if(j===0)ctx.moveTo(p.x,p.y); else ctx.lineTo(p.x,p.y); }
    ctx.strokeStyle=r.hue; ctx.lineWidth=2; ctx.globalAlpha=.8; ctx.stroke();
    ctx.beginPath(); ctx.arc(r.x,r.y,3,0,Math.PI*2); ctx.fillStyle='#fff'; ctx.globalAlpha=1; ctx.fill();
    if (r.y<=r.tY || r.vy>=-.2){
      r.life=0;
      confettiBurst({count:170+(Math.random()*80|0),pow:1.15,originX:r.x/innerWidth,originY:r.y/innerHeight,spread:Math.PI*1.2});
    }
  }
...
document.getElementById('resetHotspot').addEventListener('click', () => {
  if (confirm("Alles zurücksetzen und neuen Tag beginnen?")) {
    ...
    // Anzeige sofort zurücksetzen
    numberEl.textContent = "🍸";   // vorher: "-"
